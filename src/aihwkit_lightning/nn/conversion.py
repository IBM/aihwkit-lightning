...
from torch.nn import Module, Linear, Conv2d, Sequential, LSTM

from aihwkit_lightning.exceptions import ArgumentError
from aihwkit_lightning.nn.modules.container import AnalogWrapper
from aihwkit_lightning.nn import AnalogLinear, AnalogConv2d, AnalogSequential, AnalogLayerBase, AnalogRNN
from aihwkit_lightning.simulator.configs import TorchInferenceRPUConfig

_DEFAULT_CONVERSION_MAP = {Linear: AnalogLinear, Conv2d: AnalogConv2d, Sequential: AnalogSequential, LSTM: AnalogRNN}
_DEFAULT_DIGITAL_CONVERSION_SET = {*_DEFAULT_CONVERSION_MAP.values()}


def specific_rpu_config_id(
    module_name: str, module: Module, rpu_config: TorchInferenceRPUConfig
...
def convert_to_analog(
    ...
    if module.__class__ in conversion_map and module_name not in exclude_modules:
        ...
        # this frees up memory directly
        if inplace:
            conversion_map[module.__class__].move_to_meta(module)

    # Convert children.
    convert_dic = {}
    for name, mod in maybe_converted_module.named_children():
